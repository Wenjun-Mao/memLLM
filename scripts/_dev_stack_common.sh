...
DEFAULT_DOUBAO_ENDPOINT="https://ou-han.cn/DouBao/chat_new_ai"
...
MODEL_GATEWAY_CONTAINER="memllm-model-gateway"
...
MEMLLM_MODEL_GATEWAY_PORT="${MEMLLM_MODEL_GATEWAY_PORT:-9100}"
...
load_env_file() {
  ...
  MEMLLM_MODEL_GATEWAY_PORT="${MEMLLM_MODEL_GATEWAY_PORT:-9100}"
  MEMLLM_DOUBAO_ENDPOINT="${MEMLLM_DOUBAO_ENDPOINT:-$DEFAULT_DOUBAO_ENDPOINT}"
  ...
  OLLAMA_PRELOAD_ATTEMPTS="${OLLAMA_PRELOAD_ATTEMPTS:-5}"
  OLLAMA_PRELOAD_DELAY_SECONDS="${OLLAMA_PRELOAD_DELAY_SECONDS:-5}"
  ...
  MEMLLM_MODEL_GATEWAY_BASE_URL="${MEMLLM_MODEL_GATEWAY_BASE_URL:-http://127.0.0.1:${MEMLLM_MODEL_GATEWAY_PORT}}"
